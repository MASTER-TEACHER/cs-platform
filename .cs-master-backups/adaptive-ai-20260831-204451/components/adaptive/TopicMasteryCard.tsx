  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-black text-slate-950">{topic.topic}</h3>
          <p className="mt-1 text-sm text-slate-500">
            {topic.attempts} assessed attempt{topic.attempts === 1 ? "" : "s"}
          </p>
        </div>

        <span
          className={`rounded-full px-3 py-1 text-xs font-bold capitalize ${badgeClass(topic.state)}`}
        >
          {topic.state.replace("-", " ")}
        </span>
      </div>
      ...
      <p className="mt-4 text-sm leading-6 text-slate-600">{topic.reason}</p>
