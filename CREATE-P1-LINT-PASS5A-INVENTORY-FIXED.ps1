...
foreach ($result in $results) {
  ...
  foreach ($message in $messages) {
    ...
    $suggestionsProperty =
      $message.PSObject.Properties |
      Where-Object { $_.Name -eq "suggestions" } |
      Select-Object -First 1

    if ($null -ne $suggestionsProperty -and $null -ne $message.suggestions) {
      foreach ($suggestion in @($message.suggestions)) {
        $descProperty =
          $suggestion.PSObject.Properties |
          Where-Object { $_.Name -eq "desc" } |
          Select-Object -First 1

        if ($null -ne $descProperty -and $suggestion.desc) {
