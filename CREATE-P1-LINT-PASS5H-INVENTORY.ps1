...
$report = Join-Path $env:TEMP "cs-master-p1-pass5h-inventory-$timestamp.json"
$output = Join-Path $ProjectRoot "P1-LINT-PASS5H-INVENTORY.txt"

Write-Host ""
Write-Host "CS Master - P1 Lint Pass 5H Inventory" -ForegroundColor Cyan
...
$lines.Add("CS MASTER - P1 LINT PASS 5H INVENTORY")
$lines.Add("Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')")
$lines.Add("Project: $ProjectRoot")
$lines.Add("")
$lines.Add("Purpose: exact source-aware inventory for all remaining P1 lint findings.")
...
foreach ($result in $results) {
  ...
  foreach ($message in $messages) {
    ...
    if ($sourceLines.Count -gt 0) {
      $start = [Math]::Max(1, $lineNumber - 22)
      $end = [Math]::Min($sourceLines.Count, $lineNumber + 32)
...
Write-Host "Pass 5H inventory complete." -ForegroundColor Green
Write-Host "No source files were changed." -ForegroundColor Green
Write-Host ""
Write-Host "Created:"
Write-Host $output -ForegroundColor Yellow
Write-Host ""
Write-Host "Send me P1-LINT-PASS5H-INVENTORY.txt next."