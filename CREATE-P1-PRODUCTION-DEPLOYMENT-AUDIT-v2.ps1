...
foreach ($serviceRoot in $serviceRoots) {
  ...
  foreach ($candidate in $candidateFiles) {
    $lowerName = $candidate.Name.ToLowerInvariant()
    ...
    $relative = Relative-Path $candidate.FullName
...
foreach ($scanRoot in $scanRoots) {
  ...
  foreach ($scanFile in $scanFiles) {
    $relative = Relative-Path $scanFile.FullName
    $fileLines = Get-Content -LiteralPath $scanFile.FullName -ErrorAction SilentlyContinue
