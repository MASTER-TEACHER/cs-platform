...
export default function TeacherKnowledgeMapPage() {
  const { user } = useAuth();
  const [map, setMap] = useState<ClassKnowledgeMapType | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        setMap(await getClassKnowledgeMap(user.uid));
      } finally {
        setLoading(false);
      }
    }
  ...
  if (!map) {
    return <Card>Class knowledge map unavailable.</Card>;
  }
  ...
  return (
    <div className="space-y-8">
      <Card className="border-0 bg-gradient-to-r from-teal-950 to-cyan-800 text-white">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black">Class Knowledge Map</h1>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Metric label="Students" value={map.studentCount.toString()} />
        <Metric
