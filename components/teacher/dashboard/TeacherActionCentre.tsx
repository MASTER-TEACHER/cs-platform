...
import { buildTeacherActionCentre } from "@/services/teacherActionCentreService";
import type {
...
function priorityStyle(priority: TeacherActionPriority): string {
  ...
  if (priority === "medium") return "border-amber-300 bg-amber-50 text-amber-800";
...
function ActionIcon({ kind }: { kind: TeacherActionKind }) {
  const className = "h-5 w-5";

  if (kind === "student_support") return <GraduationCap className={className} />;
  if (kind === "topic_reteach") return <BookOpenCheck className={className} />;
  if (kind === "completion") return <ClipboardList className={className} />;
  if (kind === "class_setup") return <Users className={className} />;
  if (kind === "assessment") return <Target className={className} />;

  return <BarChart3 className={className} />;
}

export default function TeacherActionCentre(
  props: TeacherActionCentreInput,
) {
  const summary = buildTeacherActionCentre(props);
    ...
    <Card className="overflow-hidden rounded-3xl border border-teal-200 p-0">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-teal-200">
              Teacher Action Centre
            </p>
            <h2 className="mt-2 text-2xl font-black">
              {summary.headline}
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/70">
              CS Master converts dashboard evidence into a prioritised teacher
              worklist. Actions are recommendations for professional judgement,
              not automatic decisions.
            </p>
          </div>

          <div className="grid grid-cols-4 gap-2">
            <Counter label="Urgent" value={summary.criticalCount} />
            <Counter label="High" value={summary.highCount} />
            <Counter label="Medium" value={summary.mediumCount} />
            <Counter label="Monitor" value={summary.lowCount} />
          </div>
        </div>
      </div>
      ...
      {summary.actions.length === 0 ? (
        <div className="flex items-center gap-3 p-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Continue collecting assessment evidence and monitoring class
              performance.
            </p>
          </div>
        </div>
        ...
        <div className="divide-y divide-slate-100">
          {summary.actions.slice(0, 8).map((action) => (
            <div
              key={action.id}
      ...
      {summary.criticalCount > 0 && (
        <div className="flex items-start gap-3 border-t border-red-100 bg-red-50 px-6 py-4 text-sm leading-6 text-red-900">
          ...
          Urgent flags should be checked against the underlying student evidence
