...
export default function TeacherCommandCentre() {
  ...
  const priorityStudents = getPriorityStudents(portfolio)
    .filter((student) => student.priority !== "none")
    .slice(0, 4);

  const primaryTopic = summary.priorityTopics[0] ?? null;
  const strongestTopic = summary.strongestTopics[0] ?? null;
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black">
              {attentionCount > 0
                ? `${attentionCount} intelligence signal${
                    attentionCount === 1 ? "" : "s"
                  } require review`
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/70">
              A single operational view of attainment, targets, evidence
              quality, curriculum priorities and intervention demand.
              Recommendations support professional judgement and should be
              checked against the underlying evidence before action.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              ...
              className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-white px-4 text-xs font-black text-teal-950"
            >
              Detailed reports
          ...
          {priorityStudents.length === 0 ? (
            <div className="mt-4 rounded-2xl bg-emerald-50 p-5">
              ...
              <p className="mt-1 text-sm text-emerald-700">
                Continue collecting evidence and monitor the next assessment
                cycle.
              </p>
            </div>
          ) : (
              ...
              {priorityStudents.map((student) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    ...
                    {student.gap !== null ? ` · gap ${student.gap}` : ""}
                  </p>
