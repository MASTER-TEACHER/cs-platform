...
function validateInput(input: CreateExamAssignmentInput) {
  ...
  if (!input.classId.trim()) throw new Error("Select a class.");
  if (!input.questionSetId.trim())
    throw new Error("A question set is required.");
  if (!input.title.trim()) throw new Error("Enter an assignment title.");
  if (
...
function sameCalendarDay(first: Date | null, second: Date): boolean {
...
async function findExistingExamAssignment(
  ...
  const assignments = snapshot.docs.map((document) =>
    convertAssignment(document.id, document.data() as FirestoreExamAssignment),
  );
...
export async function createExamAssignment(
  ...
  const existing = await findExistingExamAssignment(input);
  ...
  const reference = await addDoc(collection(db, "examAssignments"), {
    ...
    questionSetTitle: input.questionSetTitle.trim(),
    questionSetSnapshot: input.questionSetSnapshot,
    title: input.title.trim(),
    instructions: input.instructions?.trim() || "",
    dueDate: Timestamp.fromDate(input.dueDate),
    status: "active",
    totalMarks: input.questionSetSnapshot.totalMarks,
...
export async function getExamAssignmentById(
  ...
  const snapshot = await getDoc(doc(db, "examAssignments", assignmentId));
...
export async function deleteExamAssignment(
  ...
  await deleteDoc(doc(db, "examAssignments", assignmentId));
}
