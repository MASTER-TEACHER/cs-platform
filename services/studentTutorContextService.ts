...
function topicContext(
  ...
  return {
    ...
    independentEvidenceCount:
      topic.independentEvidenceCount,
    supportedEvidenceCount:
      topic.supportedEvidenceCount,
    state: topic.state,
...
/*
 * Build the Tutor course identifier from the canonical
 * qualification and exam-board values.
 *
 * This prevents a stale historical profile value such as
 * "gcse-aqa" from being presented to an A-Level learner.
 */
function canonicalCourseId(
  ...
  const normalisedQualification =
    qualification
      .trim()
      .toUpperCase();

  if (
    normalisedQualification === "A_LEVEL" ||
    normalisedQualification === "A-LEVEL" ||
    normalisedQualification === "ALEVEL"
  ) {
    return `a-level-${board}`;
  }

  if (
    normalisedQualification === "GCSE"
  ) {
    return `gcse-${board}`;
  }
...
export async function getStudentTutorContext(
  ...
  const markedExamScores =
    submissions.flatMap(
      (submission) =>
        submission?.status ===
          "marked" &&
        typeof submission.percentage ===
          "number"
  ...
  return {
    ...
    supportedEvidenceCount:
      plan.supportedEvidenceCount,

    dueForReviewCount:
      plan.dueForReviewCount,
