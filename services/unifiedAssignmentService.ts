...
export type UnifiedAssignmentKind = "resource" | "quiz" | "exam";

export type UnifiedAssignmentStatus =
  "not_started" | "in_progress" | "completed" | ExamSubmissionStatus;
...
function convertQuiz(assignment: StudentQuizAssignment): UnifiedAssignment {
  return {
    kind: "quiz",
    id: assignment.id,
...
function convertExam(
  ...
  return {
    ...
    id: assignment.id,
    title: assignment.title,
    topic: assignment.questionSetSnapshot.topic || assignment.questionSetTitle,
    description: assignment.instructions || "",
    ...
    percentage: submission?.status === "marked" ? submission.percentage : null,
    score:
      submission?.status === "marked" ? submission.totalAwardedMarks : null,
    totalQuestions: null,
    earnedXP: null,
...
export async function getUnifiedStudentAssignments(
  ...
  const examItems = await Promise.all(
    exams.map(async (assignment) =>
      convertExam(
        assignment,
        await getExamSubmission(assignment.id, studentId),
      ),
    ),
  );
